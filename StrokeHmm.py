...
def t():
    ...
    for y in lFileList[15:]:
        trainerFiles = trainingDir + '/' + y
        strokes, labels = sl.loadLabeledFile(trainerFiles)
        ourLabels = sl.labelStrokes(strokes)
        print sl.confusion(labels, ourLabels)
...
class HMM:
    ...
    def label( self, data ):
        ...
        # 0 = drawing, 1 = text
        priorDrawing = self.priors['text']
        priorLetter = self.priors['drawing']

        transitionMatrix = self.transitions
        emissionMatrix = self.emissions

        dpath = []
        lpath = []

        prevDprob = math.log(priorDrawing * emissionMatrix['drawing']['length'][data[0]['length']])
        prevLprob = math.log(priorLetter * emissionMatrix['text']['length'][data[0]['length']])

        for obs in data[1:]:
            d_emission_value = emissionMatrix['drawing']['length'][obs['length']] * emissionMatrix['drawing']['time'][obs['time']]
            dposs = [
                prevDprob + math.log(transitionMatrix['drawing']['drawing'] * d_emission_value),
                prevLprob + math.log(transitionMatrix['text']['drawing'] * d_emission_value)
            ]
            dbest = argmax(dposs)
            if dbest == 0:
                nextdpath = dpath[:]
                nextdpath.append('drawing')
            elif dbest == 1:
                nextdpath = lpath[:]
                nextdpath.append('text')

            l_emission_value = emissionMatrix['text']['length'][obs['length']] * emissionMatrix['text']['time'][obs['time']]
            lposs = [
                prevDprob + math.log(transitionMatrix['drawing']['text'] * l_emission_value),
                prevLprob + math.log(transitionMatrix['text']['text'] * l_emission_value)
            ]

            lbest = argmax(lposs)
            if lbest == 0:
                nextlpath = dpath[:]
                nextlpath.append('drawing')
            elif lbest == 1:
                nextlpath = lpath[:]
                nextlpath.append('text')

            prevDprob = dposs[dbest]
            prevLprob = lposs[lbest]

            dpath = nextdpath
            lpath = nextlpath

        pathProbs = [prevDprob, prevLprob]
        bestpath = argmax(pathProbs)
        if bestpath == 0:
            dpath.append('drawing')
            return dpath
        else:
            lpath.append('text')
            return lpath
...
class StrokeLabeler:
    def __init__(self):
        ...
        self.featureNames = ['length', 'time']
        self.contOrDisc = {'length': DISCRETE, 'time': DISCRETE}
        self.numFVals = { 'length': 2, 'time': 2}
    ...
    def confusion(self, trueLabels, classifications):
        """outputs confusion matrix in format:
        ...
        return {
            ...
            'text': {'text': text_as_text, 'drawing': text_as_drawing}
        }
...
class Stroke:
    ...
    def time( self ):
        """calculates the time taken to make the stroke"""
        first = self.points[0]
        last = self.points[-1]
